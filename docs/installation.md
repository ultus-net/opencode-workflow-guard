...
Project-memory maintenance runs opportunistically when a project index is opened, at most once per day. Superseded memories and resolved review follow-ups are retained for 90 days and then pruned; current memories are not expired just because they are old. Maintenance checkpoints and compacts the active SQLite database, and the project-memory directory is bounded to 100 MB by evicting least-recently-used inactive project databases until usage is about 80 MB. The current project's database is never selected for global eviction, and maintenance failures do not disable core guard enforcement.
