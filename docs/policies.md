...
- Automatic continuation remains synthetic. With the opt-in project setting `titleSettleWorkaround: true`, an eligible root session with unfinished owned todos briefly waits for OpenCode's native asynchronous title generation before that continuation is sent; the bounded wait never rewrites titles or provider/model parameters.
