...
| **8** | **Workspace Boundary Guard** | Blocks file tools (`edit`, `write`, `apply_patch`) **and** shell mutations (redirection `>`, `tee`, `sed -i`, `cp`/`mv`, `git apply`) from escaping the workspace root via `../` path traversal or symlinks. External repository git writes are also confined. |
...
| **10** | **Evidence-Based Verification** | Runs `npm test` (or `WORKFLOW_GUARD_VERIFY`) in an isolated, scrubbed environment with timeout controls. Blocks final "all done" completion until fresh passing verification evidence is recorded after the latest mutation. |
