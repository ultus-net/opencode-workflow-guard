...
- Protected-path/tamper matching is anchored to **live consumption surfaces** (project-root config files, the `.opencode/` control dir minus agent/command payload and plans, the live user config dir, installed guard copies) - never on filename segments anywhere. Versioned drafts (e.g. `dotfiles/.config/opencode/**`) and docs stay editable; pure shell-mutation extraction lives in `src/policies/shell-mutations.ts` and is shared by the boundary and tamper policies. Branch creation is never gated; base staleness is an audited advisory.
